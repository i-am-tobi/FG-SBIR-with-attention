...
class VGG_Network(nn.Module):
    def __init__(self, hp):
        ...
        #defining attention (Containing two Convolution layers)
    ...
    def forward(self, input, bb_box = None):
        ...
        #Input the feature vector to the attention model
        y = F.relu(self.conv1(x))
        #Defining the attention map/mask using log_softmax
        y = F.log_softmax(self.conv2(y))

        #Defining the shortcut connection
        #Fusing attention mask and main featue vector
        z = torch.mul(x,y)
        #Defining the shortcut connection between main feature and attended feature
        z = torch.add(z,x)
